...
const EditableTable = () => {
  ...
  const retrieveAllTags = () => {
    axios.get('http://localhost:5001/product_tags/')
      .then(res => {
        setData(res.data);
  ...
  const addTag = () => {
    ...
    axios.post('http://localhost:5001/product_tags/add', data)
  ...
  const onSave = (index, tag_name, tag_desc, tag_status) => {
    ...
    axios.post('http://localhost:5001/product_tags/update/' + index, newData)
      .then(res => retrieveAllTags())
  ...
  const columns = [
    {
      ...
      render: (value, row, index) => {
        if (index === editingIndex) {
          return [
            <Input key={index} disabled={editable} value={value} onChange={event => setInput(event.target.value, index, "product_tag_name")} />
          ];
        } else {
          return [
    ...
    },
    {
      ...
      render: (value, row, index) => {
        if (index === editingIndex) {
          return [
            <Input key={index} disabled={editable} value={value} onChange={event => setInput(event.target.value, index, "product_tag_description")} />
          ];
        } else {
          return [
            <Typography key={index} >{value}</Typography>
    ...
    },
    {
      ...
      render: (value, row, index) => {
        if (index === editingIndex) {
          return [
            <Switch key={index} checkedChildren={<CheckOutlined />} unCheckedChildren={<CloseOutlined />} defaultChecked={value ? true : false} 
                    disabled={editable} onChange={() => handleSwitchChange(value, index, "product_tag_active")} />
          ];
        } else {
          return [
            <Switch key={index} checkedChildren={<CheckOutlined />} unCheckedChildren={<CloseOutlined />} defaultChecked={value ? true : false} disabled />
          ];
        }
      }
    },
          ...
          return [
            <React.Fragment key={index}>
              <Button icon={<SaveOutlined />} shape={"circle"} type={"primary"} style={{ marginRight: 8 }}
                      onClick={() => onSave(row.id, row.product_tag_name, row.product_tag_description, row.product_tag_active)} />
  ...
  return (
    <div>
      <Input placeholder="Enter tag name here" value={tag.product_tag_name} type="text" onChange={handleInputChange} id="product_tag_name"
             name="product_tag_name" maxLength={22} style={{ width: 320, marginBottom: 10 }} />
      <Button style={{ marginLeft: 3 }} type={"primary"} onClick={addTag} disabled={!tag.product_tag_name} >Add</Button>
