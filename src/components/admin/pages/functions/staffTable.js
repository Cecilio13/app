...
import {Button, Table, Row, Col, Select, Input, Menu, Dropdown, Modal, Form, Space, Tag, DatePicker, Card, Timeline} from 'antd';
import { UserAddOutlined, DownOutlined, EditOutlined, EyeOutlined, RightCircleOutlined } from '@ant-design/icons';
...
function StaffTable(){   
    ...
    const columns = [
                ...
                <>
                    {
                        note.map(notes => (
                            ...
                            <Card size="small" extra={<a onClick={() => handleDeleteNote(notes._id, staff._id)}>x</a>}>
                                <p>{notes.info}</p>
                            </Card> : null
                            )
                        )
        ...
        },
        {
            ...
            render: status => (
                ...
                ? <Button shape="round" type="primary" size="small"> </Button>
                : <Button shape="round" type="danger" size="small"> </Button>),
            filters: [
                {
                    text: 'Active',
                    value: true
                ...
                return [
                    <>
                        <Row>
                            <Col span={12}>
                                <span>
                                    <a key={_id} onClick={() => handleViewEdit(_id)} style={{float: 'right'}}><EditOutlined style={{fontSize: 20}}/> Edit</a>
                                </span>
                            </Col>
                            <Col span={12}>
                                <span>
                                    <a key={_id} onClick={() => viewLogs(staff)}> <EyeOutlined style={{fontSize: 20}}/> Logs</a>
                                </span>
                            </Col>
                        </Row>                                                
                    </>
    ...
    const handleDeleteNote = (note_id, staff_id) => {
        ...
        axios.post('http://localhost:5001/staff/delete/note', save)
            .then(res => renderData());
    }
    ...
    const handleFinishEdit = (values) => {
        ...
        axios.post('http://localhost:5001/staff/edit', save)
            .then(res => renderData());
    ...
    //for display data
    useEffect(() => {
        axios.get('http://localhost:5001/staff')
        .then(res => {
    ...
    const expandedRowRender = expandedRowKey => {
        console.log(filteredStaff, 'this filtered staff note');
        console.log('expandedRowKey selected: ', expandedRowKey);        
        setLogs(expandedRowKey.action_log);

        const columns = [            
            { title: 'Birthdate', dataIndex: 'birthday', key: 'birthday',align: 'center'},
    ...
    function handleMenuClick(e) {
        console.log(e.key, 'this is key of menu');
        if(e.key === '1'){
            const save = [];
            ...
            console.log(save, 'this is save');
            axios.post('http://localhost:5001/staff/update/status', save)
                .then(res => renderData());
        }
        else if(e.key === '2'){
            const save = [];
    ...
    const handleFinishNote = (value) => {               
        console.log('this is note: ', value);
        console.log('this is selected key in note', selectedRowKeys);
        const save = []
        ...
        axios.post('http://localhost:5001/staff/add_note', save)
            .then(res => renderData())
    ...
    const handleFinishAdd = (values) => {
        ...
        console.log('this is date: ', dateToday);
        console.log('this is birhtday: ', dateBirthdayAdd);
        ...
        axios.post('http://localhost:5001/staff/add', staff)
            .then(res => renderData());
    ...
    function onChange(date, dateString) {
        ...
        console.log('birthday: ', dateBirthdayAdd);
        ...
        <>
            <section>
                <div>
                    <Space direction="vertical">    
                        <Button shape="round" type="primary" size="large" onClick={showModalAdd}><UserAddOutlined></UserAddOutlined>Add</Button>  
                    </Space>
                    <Modal title="Add Staff" visible={visibleAdd} footer={[
                        <Modal footer={null}/>
                    ]}>
                        <Form {...formItemLayout} onFinish={handleFinishAdd}>
                            ...
                            <Form.Item name="birthday" label="Birhdate"
                                rules={[
                                    {
                                        required: true,
            ...
            <section>
                <Row>
                    ...
                    <Col span={12}>
                        <Search placeholder="Enter Search" style={{width: 200, float: 'right'}} onChange={e => setSearch(e.target.value)}/>
                    </Col>
                </Row>

                <Table className="components-table-demo-nested" rowKey={staff=>staff._id} rowSelection={rowSelection} expandable={{expandedRowRender}} columns={columns} 
                dataSource={filteredStaff} pagination={{defaultPageSize: 10, position: ['bottomCenter']}} ></Table>
            </section> 

            <Modal title="Add Note" visible={visibleNote} footer={[<Modal footer={null}/>]}> 
                <Form onFinish={handleFinishNote}>
                    <Form.Item name="note" key={1}
                            rules={[
                                {
                                    required: true,
            ...
            <Modal title="Edit Staff" visible={visibleEdit} footer={[<Modal footer={null}/>]}>
                <Form {...formItemLayout} onFinish={handleFinishEdit}>
                    ...
                    <Form.Item name="value_birthday" label="Birhdate">
                            <DatePicker onChange={onChangeEditBirthday} placeholder={editValue.birthday}></DatePicker>
            ...
            <Modal title="User Logs" visible={visibleLogs} onOk={handleOkLogs}>                    
                <DisplayLogs/>                    
            </Modal>
